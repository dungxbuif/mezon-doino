...
export class BotHandler {
  ...
  private async deliveryDeptConfirmEvent(message: ChannelMessage) {
    if (DeptCommand.isShowConfirm(message.content.t)) {
      this.emitter.emit(AppEventEnum.DEPT_LIST_CONFIRM, message);
    }
  }
  ...
  private async helpCommandEvent(message: ChannelMessage) {
    ...
    if (
      ...
      const messageToSend = replyMessageGenerate(
        {
          messageContent: OrderCommand.helpContent,
